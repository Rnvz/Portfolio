...
export function Work() {
  ...
  const handleUp = () => {
    if (view === 'grid' || view === 'index') setSelectedIndex(i => Math.max(0, i - 1))
    else if (view === 'detail') {
  ...
  const handleDown = () => {
    if (view === 'grid' || view === 'index') setSelectedIndex(i => Math.min(PROJECTS.length - 1, i + 1))
    else if (view === 'detail') {
      const el = document.getElementById('detail-scroll')
      if (el) el.scrollTop += 50
    }
  ...
  const handleLeft = () => {
    if (view === 'grid' || view === 'detail') setSelectedIndex(i => Math.max(0, i - 1))
  }

  const handleRight = () => {
    if (view === 'grid' || view === 'detail') setSelectedIndex(i => Math.min(PROJECTS.length - 1, i + 1))
  }

  const handleCenter = () => {
    if (view === 'idle') setView('grid')
    else if (view === 'grid') setView('detail')
    else if (view === 'index') setView('detail')
  }
    ...
    <section ref={sectionRef} className="relative w-full min-h-screen bg-[var(--scene-work)] flex flex-col justify-center px-[var(--section-px)] py-[var(--section-py)]">
      ...
      <div className="work-content max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16 items-center pt-16 md:pt-0">
          ...
          <div className="bg-[var(--surface)] border border-[var(--border-mid)] rounded-[3rem] w-full max-w-[540px] h-[820px] flex flex-col p-6 shadow-2xl relative">
            ...
            <div className="w-full h-[440px] bg-[#0a0a0a] rounded-2xl border border-[var(--border)] relative overflow-hidden flex flex-col shadow-inner shrink-0">
              ...
              <div className="flex justify-between items-center px-5 py-3 text-[10px] font-medium text-[var(--text-secondary)] font-mono uppercase shrink-0 relative z-10 bg-[#0a0a0a]/90 backdrop-blur-sm border-b border-white/[0.05]">
                <span>{view === 'idle' ? 'Projects' : `${String(selectedIndex + 1).padStart(2, '0')} / ${String(PROJECTS.length).padStart(2, '0')}`}</span>
        ...
        <div className="order-1 md:order-2 flex flex-col items-start md:text-left w-full h-auto md:h-[820px]">
          <AnimatePresence mode="wait">
            {(view === 'idle' || view === 'grid' || view === 'index') ? (
              <motion.div
                key="overview"
